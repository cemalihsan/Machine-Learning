...
def vector_read():
    ...
    with open('data/wordVectors.txt') as f:
        for line in f:
            ...
            vec_str = line.split(' ')
            vec_num = []
            for number in vec_str:
                vec_num.append(float(number))

            vector_array.append(vec_num)
            counter.append(line_number)
            line_number += 1
    f.close()

'''
def combine_array(array1, array2, array3, input_array):
    count = 0
    for entry in array1:
        input_array.append([array1[count], array2[count], array3[count]])
        count = count + 1


def array_to_dict(list_array):
    line_num = 1
...
#dict_array = dict(zip(vocab_array, vector_array))
#print(dict_array['george'])
